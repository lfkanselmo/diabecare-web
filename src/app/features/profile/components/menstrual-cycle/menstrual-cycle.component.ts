...
export class MenstrualCycleComponent implements OnInit {
    ...
    this.cycleService.register(patientId, this.startForm.getRawValue()).subscribe({
      next: (data) => {
        ...
        this.notificationService.success(
          this.transloco.translate('profile.cycle.periodRegisteredSuccess'),
        );
        this.saving.set(false);
      },
      error: () => {
        this.notificationService.danger(
          this.transloco.translate('profile.cycle.periodRegisterError'),
        );
        this.saving.set(false);
      },
    });
  ...
  onSaveDayEntry(): void {
    ...
    const symptoms: SymptomInput[] = Array.from(this.selectedSymptoms().entries()).map(
      ([symptom, severity]) => ({ symptom, severity }),
    );

    this.cycleService
      .registerDayEntry(patientId, {
      ...
      .subscribe({
        next: () => {
          this.notificationService.success(
            this.transloco.translate('profile.cycle.todayEntrySaved'),
          );
          this.saving.set(false);
          this.loadStatus();
        },
        error: () => {
          this.notificationService.danger(
            this.transloco.translate('profile.cycle.todayEntryError'),
          );
          this.saving.set(false);
        },
      });
  ...
  onFinishPeriod(): void {
    ...
    this.cycleService.finishPeriod(patientId, this.finishForm.getRawValue()).subscribe({
      next: (data) => {
        this.status.set(data);
        this.notificationService.success(
          this.transloco.translate('profile.cycle.periodFinishedSuccess'),
        );
      ...
      error: (err) => {
        const message =
          err?.error?.message ?? this.transloco.translate('profile.cycle.periodFinishError');
  ...
  onJumpToFinishPeriod(): void {
    this.confirmFinish.set(true);
    document
      .getElementById('finish-period-section')
