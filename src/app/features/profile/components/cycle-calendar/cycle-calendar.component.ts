...
export class CycleCalendarComponent implements OnChanges {
    ...
    readonly phaseConfig: Record<CyclePhase, PhaseConfig> = {
        MENSTRUATION: { label: 'Menstruación', color: '#EF5350', lightColor: '#FFEBEE', days: '1-5', icon: '🩸' },
        FOLLICULAR: { label: 'Fase folicular', color: '#66BB6A', lightColor: '#E8F5E9', days: '6-13', icon: '🌱' },
        OVULATION: { label: 'Ovulación', color: '#42A5F5', lightColor: '#E3F2FD', days: '14', icon: '✨' },
        LUTEAL_EARLY: { label: 'Lútea temprana', color: '#FFA726', lightColor: '#FFF3E0', days: '15-21', icon: '🌙' },
        LUTEAL_LATE: { label: 'Lútea tardía', color: '#FF7043', lightColor: '#FBE9E7', days: '22-28', icon: '⚡' }
    };

    ngOnChanges(): void {
        if (this.status) {
            this.buildWheelChart();
            this.buildCalendar();
        }
    }

    private buildWheelChart(): void {
            ...
            series: [
                {
                    ...
                    data: phases.map((p, i) => ({
                        ...
                        itemStyle: {
                            ...
                            borderColor: p.phase === currentPhase ? '#fff' : 'transparent',
    ...
    private buildCalendar(): void {
        ...
        for (let i = 0; i < startPad; i++) {
            this.calendarDays.push({ date: null, phase: null, isToday: false, isPredicted: false });
        }
        ...
        for (let d = 1; d <= lastDay.getDate(); d++) {
            ...
            this.calendarDays.push({ date: d, phase, isToday, isPredicted });
        }
    }
    ...
    getDayColor(day: CalendarDay): string {
        ...
        return this.phaseConfig[day.phase].lightColor;
    }

    getDayBorderColor(day: CalendarDay): string {
        if (!day.phase) return 'transparent';
        return this.phaseConfig[day.phase].color;
    }
}
