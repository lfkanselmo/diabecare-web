...
export class GlucoseRemindersComponent implements OnInit {
      ...
      .subscribe({
        next: (reminder) => {
          this.reminders.update((list) =>
            [...list, reminder].sort((a, b) => a.reminderTime.localeCompare(b.reminderTime)),
          );
          this.form.reset();
          this.notificationService.success(this.transloco.translate('profile.reminders.added'));
        },
        error: () =>
          this.notificationService.danger(this.transloco.translate('profile.reminders.addError')),
      });
  }
  ...
  onToggle(reminder: GlucoseReminderResponse): void {
    ...
    this.reminderService.toggle(patientId, reminder.id, !reminder.enabled).subscribe({
      ...
      error: () =>
        this.notificationService.danger(this.transloco.translate('profile.reminders.toggleError')),
    });
  }
